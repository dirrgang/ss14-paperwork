...
def render_documents_yaml(documents: Iterable[PaperDocument]) -> str:
    ...
    def ensure_category_id(category: str) -> str:
        identifier = category_id_map.get(category)
        if identifier:
            return identifier

        base_slug_source = normalise_component(category)
        base_slug = ensure_document_slug(base_slug_source)
        slug = allocate_slug(base_slug, used_slugs, slug_counters)

        base_id = to_pascal_case(slug)
        base_id = ensure_document_id(base_id)
        identifier = allocate_id(base_id, used_ids, id_counters)
        category_id_map[category] = identifier
        return identifier
    ...
    for doc in docs:
        ...
        if doc.categories:
            ...
            category_value = ensure_category_id(primary_category)
